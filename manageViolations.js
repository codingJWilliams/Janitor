...
var url = require("./token.json").mongo;
...
exports.setClient = (_client) => {
  client = _client;
  modlog = _client.guilds.get("300155035558346752").channels.get("380284981773336576")
};

exports.getViolations = async(id) => db ? await db.collection("violations").find({
  uid: id,
  archived: false
}).toArray() : null;
...
exports.indicate = (text) => {
  whoaaaaaaa.map(whoa => {
    text = text.toLowerCase().replaceAll(whoa, "__" + whoa + "__")
  });
  return text;
}
...
exports.hasViolation = async(message) => {
  if (db) {
    var fails = whoaaaaaaa.some((banned) => {
      return message.content.toLowerCase().indexOf(banned) !== -1;
    });
    if (fails) {
...
exports.archiveAll = async(member) => await db.collection("violations").updateMany({
  uid: member.id,
  archived: false
}, {
  $set: {
...
exports.addViol = async(member, content, auto) => {
  if (auto) {
    db.collection("violations").insertOne({
      uid: member.id,
      archived: false,
      manual: false,
